...
UI = {
  ...
  initialize: function() {
    var _this = this;
    document.addEventListener('DOMNodeInsertedIntoDocument', this._insert.bind(this), true);
    return window.addEventListener('load', function() {
      if (typeof UI.beforeload === "function") {
        UI.beforeload();
      }
      _this.load();
...
UI.Context = (function(_super) {
  ...
  Context.prototype.onAdded = function() {
    ...
    if (this._parent) {
      this._parent.removeEventListener('contextmenu', this.$close);
    }
    this._parent = this.parentNode;
    return this._parent.addEventListener('contextmenu', this._open.bind(this));
  };

  Context.prototype.initialize = function() {
    document.addEventListener(UI.Events.action, this._close.bind(this));
    document.addEventListener('contextmenu', this._close.bind(this));
...
Component = (function(_super) {
  ...
  Component.prototype.initialize = function() {
    var method, selector, type, _ref1, _ref2;
    _ref1 = this.events;
    for (type in _ref1) {
      method = _ref1[type];
      if (type.match(/\s/)) {
        ...
        this.delegateEventListener(type, selector, this[method].bind(this));
      } else {
        if (UI.Events[type]) {
          type = UI.Events[type];
        }
        this.addEventListener(type, this[method].bind(this));
      }
    }
    if (this.markup) {
      return UI._build.call(this, this.markup, this);
    }
  };
